...
public class Preferabli {
    ...
    ///   - onCompletion: returns ``User`` if the call was successful. *Returns on the main thread.*
    ///   - onFailure: returns ``PreferabliException``  if the call fails. *Returns on the main thread.*
    ...
    /// Get all the questions and choices needed to run a Guided Rec. Present the quiz to the user, then pass the answers to ``Preferabli/getGuidedRecResults(selected_choice_ids:price_min:price_max:collection_id:onCompletion:onFailure:)`` to get results.
    /// - Parameters:
    ...
    /// Get a list of foods to choose from to be used in ``getRecs(product_type:price_min:price_max:collection_id:style_ids:food_ids:onCompletion:onFailure:)``.
    /// - Parameters:
    ///   - force_refresh: pass true if you want to force a refresh from the API and wait for the results to return. Otherwise, the call will load locally if available and run a background refresh only if one has not been initiated in the past 5 minutes. Defaults to *false*.
